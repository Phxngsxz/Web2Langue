    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          <span className="hidden sm:inline">{languageNames[currentLang]?.flag}</span>
          <span className="font-medium">{languageNames[currentLang]?.name}</span>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="min-w-[140px] bg-white border-gray-200">
        {locales.map((locale) => (
          <DropdownMenuItem
            key={locale}
            onClick={() => switchLanguage(locale)}
            className={`cursor-pointer flex items-center gap-2 transition-colors duration-200 ${
              locale === currentLang ? "bg-gray-100 text-black font-medium" : "hover:bg-gray-50 text-gray-700"
            }`}
          >
